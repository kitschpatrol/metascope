...
import { parsePubliccode } from '../../src/lib/parsers/publiccode-yaml-parser'
...
describe('parsePubliccode', () => {
	...
	it('should return undefined for invalid YAML', () => {
		expect(parsePubliccode(':::not yaml:::')).toBeUndefined()
	})

	it('should return undefined for non-object YAML', () => {
		expect(parsePubliccode('just a string')).toBeUndefined()
	})

	it('should return undefined for object without name or url', () => {
		expect(parsePubliccode('foo: bar\nbaz: 42')).toBeUndefined()
	})
